...
function updateQualityValue(){
  ...
  const out=document.getElementById("qualityValue");
  if(!el||!out)return;
  out.textContent=el.value+"%";
}
updateQualityValue();
if(quality){
  quality.addEventListener("input",updateQualityValue,true);
  quality.addEventListener("change",updateQualityValue,true);
  quality.addEventListener("pointermove",updateQualityValue,true);
}
if(width)width.addEventListener("input",()=>qualityPreset.value="custom");
if(height)height.addEventListener("input",()=>qualityPreset.value="custom");
...
document.addEventListener("input",e=>{
  if(e.target&&e.target.id==="quality")updateQualityValue();
},true);
...
function canvasToGifBlob(canvas){
  return new Promise((resolve,reject)=>{
    ...
    const gif=new GIF({
      workers:2,
      quality:Math.max(1,Math.min(30,Math.round((100-Number(quality.value))/3+1))),
      width:canvas.width,
...
async function convertOne(i){
 ...
 try{
   ...
   if(format.value==="image/bmp"){
     ...
     blob=await canvasToBlob(canvas,format.value,Number(quality.value)/100);
   }
   x.result=blob;
